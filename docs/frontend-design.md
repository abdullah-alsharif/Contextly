...
|---|---|---|
| Sidebar | `sidebar.tsx` | `w-[272px]`, brand header + search icon (Cmd/Ctrl+K on desktop, drawer header on mobile), "New Conversation" primary CTA (chat is entered through it — no separate Chat nav item), nav (Documents / Settings), conversation list (Pinned / Recents sections + Archived view), user chip (name, falling back to email) |
