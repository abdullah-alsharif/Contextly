...
| Chat bubble | `chat-message.tsx` | AI: `surface-container-low` card, `rounded-xl rounded-tl-none`, 1px border; user: solid `secondary` bg white text; both bubbles shrink-wrap their content (max 80% of the chat column); typing indicator = 3 bouncing dots + "Analyzing documents…" |
