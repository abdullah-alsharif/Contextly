...
| Top bar | `topbar.tsx` | Search field (rounded, focus ring), global Upload button, notifications + avatar actions |
...
| Docs table | `document-table.tsx` | Border-less rows, horizontal `1px` dividers, hover `#F8FAFC`, row actions (kebab) appear on hover, `body-sm` density, pagination footer |
...
| Source viewer | `source-viewer.tsx` | Right panel `w-80`: card w/ primary-accented header `[1] filename · Page N`, excerpt with `<mark>` highlighted span, "Open Document" link |
